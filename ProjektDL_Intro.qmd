...
tweed2beta <- function(x = c(1,1,1,1,1)) {
  # Tweedback-Ergebnis Schätzung Anteil Kaffeetrinker:innen in Parameter Beta Verteilung umrechnen
  # Input:
  # x[A:E]: Anzahl Antworten je Alternative A bis E
  
  # Geschätzte Anteile (Klassenmitte)
...
::: fragment
<br>
Das Ergebnis können Sie z. B. mit `length(...)` oder `mean(...)` überprüfen.
...
# Hinweis: eigene Funktion, mehr dazu später
...
::: footnote
*Tipp:* Mit der Pipe `|>` können Sie mehrere Liniengrafiken übereinander legen. \
*Hinweis:* Im Code müssen noch verschiedene Anpassungen vorgenommen werden.
:::
::::: columns
::: {.column width=50%}
```{webr-r}
...
aprior <- 1
bprior <- 1
# Dichtevektor für die Prior-Verteilung erzeugeh
...
apost <- 1
bpost <- 1
# Dichtevektor für die Posterior-Verteilung erzeugeh
dprior <- dbeta(ppi, shape1 = a, shape2 = b)
